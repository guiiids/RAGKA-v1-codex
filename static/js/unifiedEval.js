          ...
          if (window.addBotMessage) {
            if (result.error) {
              ...
              if (activeMode === MODES.COMPARE) {
                ...
                // Display sources if available - using centralized utility
              if (result.sources && result.sources.length > 0) {
                // Store sources for backward compatibility
                window.lastSources = result.sources;
                
                // Use centralized sources utility
                const sourcesHtml = window.addSourcesUtilizedSection(result.sources, requestId);
                if (sourcesHtml) {
                  window.addBotMessage(sourcesHtml);
                }
              }
